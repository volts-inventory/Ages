...
//! caller. The bulk of the catastrophe code lives here; the
//! sibling `damage` / `cells` / `factors` / `triggers` modules
//! supply the per-cell plumbing this dispatcher composes.
...
use super::cells::{
    apply_to_cell_and_neighbors, densest_claimed_cell, deterministic_cell_pick,
};
use super::damage::{
    apply_resistance_and_dormancy, asteroid_radiation_boost, catastrophe_cell_conditions,
    ice_age_temp_drop_k, solar_flare_radiation_boost,
};
use super::factors::{disease_severity_factor, ice_age_severity_factor, volcanic_cooldown_factor};
use super::kind::CatastropheKind;
use super::record::CatastropheRecord;
use super::triggers::{
    asteroid_fires, disease_fires, ice_age_fires, solar_flare_fires, volcanic_fires,
};
use super::{
    ASTEROID_COOLDOWN_TICKS, ASTEROID_POP_LOSS, DISEASE_COOLDOWN_TICKS, DISEASE_POP_LOSS,
    ICE_AGE_COOLDOWN_TICKS, ICE_AGE_POP_LOSS, SOLAR_FLARE_COOLDOWN_TICKS, SOLAR_FLARE_POP_LOSS,
    VOLCANIC_COOLDOWN_TICKS, VOLCANIC_POP_LOSS,
};
...
mod tests {
    use super::*;
    use super::super::DISEASE_AGE_FLOOR_TICKS;
