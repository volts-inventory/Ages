//! War resolution: strength calculation, casualty math, per-cell
//! flip logic.
//!
//! Periodic per-pair check (every `CONFLICT_CHECK_TICKS = 75`
//! ticks); strength weighted by population × literacy × hierarchical
//! composite × tool-war multiplier × surplus modifier. Loser takes a
//! population hit per disputed cell and surrenders cells whose
//! cohort drops below `CELL_FLIP_FLOOR`.

use super::grudge::{GRUDGE_BUMP_LOSER, GRUDGE_BUMP_WINNER};
use crate::Civ;
...
pub(crate) const HIER_W_COSMOLOGY: (i64, i64) = (40, 100);
pub(crate) const HIER_W_RELIGION: (i64, i64) = (20, 100);
pub(crate) const HIER_W_KINSHIP: (i64, i64) = (25, 100);
pub(crate) const HIER_W_ECONOMIC: (i64, i64) = (15, 100);
...
pub fn resolve(a: &mut Civ, b: &mut Civ, tick: u64) -> Option<ConflictOutcome> {
    ...
    // without it triggering combat. Asymmetric alliances are not
    // allowed in this model — both sides must mirror the flag
    // (the formation rule in `propose_alliance` always inserts
    // both sides; dissolution always removes both sides). The
    // gate is therefore an AND so a unilaterally-set flag (e.g.
    // from a corrupted save or a partial rollback) doesn't
    // suppress war.
