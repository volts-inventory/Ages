...
| `SolarInsolation` | Diagnostic per-cell stellar insolation = irradiance × latitude incidence × cloud clarity. | Additive substrate for the **photonic** lever; rewritten each tick, no legacy reader. ([`insolation.rs`](../sim/physics/src/insolation.rs); see [archetype.md](archetype.md)) |
| `TidalStress` | Diagnostic per-cell tidal stress = aggregate moon pull × gravity, latitude-banded. | Additive substrate for the **gravitational** lever. ([`tidal_stress.rs`](../sim/physics/src/tidal_stress.rs); see [archetype.md](archetype.md)) |
| `SurfaceRadiation` | Diagnostic per-cell ionizing radiation = radiogenic crust floor + magnetosphere-shielded cosmic flux (pole-rising). | Additive substrate for the **nuclear** lever. ([`surface_radiation.rs`](../sim/physics/src/surface_radiation.rs); see [archetype.md](archetype.md)) |
